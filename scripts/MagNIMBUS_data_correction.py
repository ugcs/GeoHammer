...
def filter_magnetic(data):
    # Shift vector channels up by one row to align with scalar timing
    data['Bx'] = data['Bx'].shift(-1)
    data['By'] = data['By'].shift(-1)
    data['Bz'] = data['Bz'].shift(-1)

    # Drop held-over duplicate samples, then interpolate the resulting gaps
    ...
    # Synthetic scalar magnitude, used to normalise the filtered vector channels
...
def filter_heading(data):
    ...
    # Decompose heading into sin/cos to avoid the 0/360 discontinuity
...
def select_for_training(data):
    ...
    # Exclude the strongest mag signals (top/bottom decile) so they don't dominate the regression
    lower_percentile = np.percentile(tmi, 10)  # may need adjusting
    upper_percentile = np.percentile(tmi, 90)  # may need adjusting
    ...
    # Extend each kept point by +/-1 row so transitions in/out of anomalies stay represented
    window_size = 1
    ...
    print(f"Percent of dataset used for model: {dataset_percent:.1f}%")  # target: >50%
